...
def parse_coo(section):
    ...
    # Vérifier si la liste coo a au moins deux éléments avant d'essayer d'accéder à coo[1]
    if len(coo) >= 2:
        deg = coo[0]
        minutes = coo[1]
        return (deg, minutes)
    else:
        # Gérer le cas où la liste coo n'a pas assez d'éléments
        return None
...
def positions_extremes(liste_infos):
    ...
    return {"max_nord": coord_nord, "min_sud": coord_sud, "max_est": coord_est, "min_ouest": coord_ouest}
            ...
            for trame in p['gga']:
                ...
                if altitude is not None:
                    try:
                        ...
                        continue  # Ignorer les valeurs d'altitude non valides
...
def premier_dernier_gga(liste_trames):
    ...
    """
    trame = extract('analyse_de_trames_NMEA_GPS/trame.txt')  
...
resultat_premier_dernier_gga = extract('trame.txt')
